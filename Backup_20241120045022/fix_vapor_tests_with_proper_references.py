# Script Name: fix_vapor_tests_with_proper_references.py
# Purpose: Fix Vapor 4 test compatibility, ensuring proper naming and access.
...
def fix_test_file(original_path, backup_path):
    ...
    updated_lines = []
    for line in lines:
        # Fix double `HTTPStatus` typo
        line = line.replace("HTTPStatusHTTPStatus", "HTTPStatus")

        # Update response.status
        if "response.status" in line:
            line = line.replace("response.status", "response.status")

        # Update .ok and .notFound
        if ".ok" in line or ".notFound" in line:
            line = line.replace(".ok", "HTTPStatus.ok")
            line = line.replace(".notFound", "HTTPStatus.notFound")

        # Ensure safe access to response.body.string
